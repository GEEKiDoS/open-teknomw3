...
namespace game_console
{
	namespace
	{
		...
		void draw_input()
		{
			...
			draw_input_box(1, con_inputBoxColor->current.vector);
			draw_input_text_and_over("Open TeknoMW3: " VERSION ">", color_s1);
			...
			if (matches.size() > 24)
			{
				draw_hint_box(1, con_inputHintBoxColor->current.vector);
				draw_hint_text(0, utils::string::va("%i matches (too many to show here)", matches.size()),
				               con_inputDvarMatchColor->current.vector);
			}
			else if (matches.size() == 1)
			{
				...
				draw_hint_box(line_count, con_inputHintBoxColor->current.vector);
				draw_hint_text(0, matches[0].data(),
				               dvar
					               ? con_inputDvarMatchColor->current.vector
					               : con_inputCmdMatchColor->current.vector);
				...
				if (dvar)
				{
					...
					draw_hint_text(0, game::Dvar_ValueToString(dvar, dvar->current),
					               con_inputDvarValueColor->current.vector, offset);
					draw_hint_text(1, "  default", con_inputDvarInactiveValueColor->current.vector);
					draw_hint_text(1, game::Dvar_ValueToString(dvar, dvar->reset),
					               con_inputDvarInactiveValueColor->current.vector, offset);
				}
			...
			{
				draw_hint_box(static_cast<int>(matches.size()), con_inputHintBoxColor->current.vector);
		...
		void draw_output_scrollbar(const float x, float y, const float width, const float height, output_queue& output)
		{
			...
			draw_box(_x, y, 10.0f, height, con_outputBarColor->current.vector);
			...
			draw_box(_x, y, 10.0f, _height, con_outputSliderColor->current.vector);
		}
		...
		void draw_output_text(const float x, float y, output_queue& output)
		{
			...
			for (auto i = 0u; i < con.visible_line_count; i++)
			{
		...
		void draw_output_window()
		{
			con.output.access([](output_queue& output)
			{
				draw_box(con.screen_min[0], con.screen_min[1] + 32.0f, con.screen_max[0] - con.screen_min[0],
				         (con.screen_max[1] - con.screen_min[1]) - 32.0f, con_outputWindowColor->current.vector);
	...
	public:
		void post_load() override
		{
			...
			// disable default rcon console
			utils::hook::nop(0x5B54C5, 5);
			...
			{
				// add our dvars
				con_inputBoxColor = game::Dvar_RegisterVec4("con_inputBoxColor", 0.2f, 0.2f, 0.2f, 0.9f, 0.0f, 1.0f,
					game::DVAR_ARCHIVE,
					"color of console input box");
				con_inputHintBoxColor = game::Dvar_RegisterVec4("con_inputHintBoxColor", 0.3f, 0.3f, 0.3f, 1.0f,
					0.0f, 1.0f,
					game::DVAR_ARCHIVE, "color of console input hint box");
				con_outputBarColor = game::Dvar_RegisterVec4("con_outputBarColor", 0.5f, 0.5f, 0.5f, 0.6f, 0.0f,
					1.0f, game::DVAR_ARCHIVE,
					"color of console output bar");
				con_outputSliderColor = game::Dvar_RegisterVec4("con_outputSliderColor", 1.0f, 0.8f, 0.0f, 1.0f,
					0.0f, 1.0f,
					game::DVAR_ARCHIVE, "color of console output slider");
				con_outputWindowColor = game::Dvar_RegisterVec4("con_outputWindowColor", 0.25f, 0.25f, 0.25f, 0.85f,
					0.0f,
					1.0f, game::DVAR_ARCHIVE, "color of console output window");
				con_inputDvarMatchColor = game::Dvar_RegisterVec4("con_inputDvarMatchColor", 1.0f, 1.0f, 0.8f, 1.0f,
					0.0f,
					1.0f, game::DVAR_ARCHIVE, "color of console matched dvar");
				con_inputDvarValueColor = game::Dvar_RegisterVec4("con_inputDvarValueColor", 1.0f, 1.0f, 0.8f, 1.0f,
					0.0f,
					1.0f, game::DVAR_ARCHIVE, "color of console matched dvar value");
				con_inputDvarInactiveValueColor = game::Dvar_RegisterVec4(
					"con_inputDvarInactiveValueColor", 0.8f, 0.8f,
					0.8f, 1.0f, 0.0f, 1.0f, game::DVAR_ARCHIVE,
					"color of console inactive dvar value");
				con_inputCmdMatchColor = game::Dvar_RegisterVec4("con_inputCmdMatchColor", 0.80f, 0.80f, 1.0f, 1.0f,
					0.0f,
